            ...
            if (lockDb)
            {
                ...
                if (nRefCount == 0)
                {
                    ...
                    if (mi != bitdb.mapFileUseCount.end())
                    {
                        ...
                        printf("Flushed wallet.dat %sms\n", std::to_string(GetTimeMillis() - nStart).c_str());
                    }
                }
            }
        }
...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    // found.
    int64 now = GetTime();
    std::string newFilename = strprintf("wallet.%s.bak", std::to_string(now).c_str());
    ...
    printf("Salvage(aggressive) found %s records\n", std::to_string(salvagedData.size()).c_str());
